...
class Block
{
	...
	public Block(int x, int y){//생성자 만들어줫음.
		this.x = x;
		this.y = y;
	}
	//해당 포인트맛큼 감산
	public void move(int xPlus, int yPlus){//move 를 했을 경우 그값에 기존에 있던 x.y 을 더한다.
		this.x += xPlus;
		this.y += yPlus;
	}
	//X포인트 반환
...
public class Item 
{
	JPanel[] panel;		//판넬 을 만들어준다.
	Block[] block; 		//현재포인트,// 기존 x,y의 값과 이동 했을떄의 x.y의 새로운값.
	Block[][] block_info;		//각 각도별 포인트정보
	//앞의배열 0~3-> 각도 0-0도 1-90도 2-180도 3-270도
	//뒤의 배열은 판넬 갯수
	Block currentXY; // Block  를 만들고 currentXY 해준다.
	int cnt;				//총판넬개수
	int angle;				//총각도개수
