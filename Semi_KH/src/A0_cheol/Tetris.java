...
//	public static boolean isRight = false;		//오른쪽여부
	...
	public Tetris(String str){
		//========== 기본설정 셋팅 시작 ===========
		this.setTitle(str);
		this.xCnt = 14;//전체 패널 x값
		this.yCnt = 25;//전체 패널 y 값.
		this.area = 20;// 도형을 사용하기  위해서 만들 어논것.
		this.width = this.xCnt * this.area;// 전체 페널의  가로의 길이
		this.height = this.yCnt * this.area;// 전페 패널의 세로의 길이./
		
		this.itemList = new ArrayList<Item>();
		// item어레 이 리스트 만들었다  새롭게 지정해줘야 사용 가능.
		this.background = new JPanel[this.xCnt][this.yCnt];// 플레이 패너를 만들었다.
		this.grid = new boolean[this.xCnt][this.yCnt];//
		//그냥 공주소.
		this.rnd = new Random(System.currentTimeMillis());//천분의 일초 로 사용 되는거,.,
	
		this.fc = this.getContentPane();

		this.center = new JPanel();//패널 만들고.
		this.center.setSize(this.width, this.height);
		this.center.setLayout(null);
		this.center.setBackground(new Color(224,0,0));//
		this.fc.add(this.center, "Center");

		this.addKeyListener(this);
		this.setBounds(200,200,this.width+8,this.height+13);//전페 프레임의 크기및위치.
